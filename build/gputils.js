...
var GPAxis = React.createClass({displayName: 'GPAxis',
  render: function() {
...
var GPList = React.createClass({displayName: 'GPList',
  render: function() {
    var delGP = this.props.delGP;
    var gplist = this.props.GPs.map(function (gp) {
      return (React.DOM.tr( {key:gp.id}, 
                React.DOM.td( {className:"tr"+gp.id}, gp.id),React.DOM.td(null, cfs[gp.cf].name),React.DOM.td(null, gp.params[0].toFixed(2)),React.DOM.td(null, gp.params[1].toFixed(2)),React.DOM.td(null, React.DOM.button( {onClick:delGP(gp.id)}, "remove"))
              ));
    });
    return (React.DOM.tbody(null, gplist));
  }
});
